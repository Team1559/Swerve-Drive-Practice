...
public class SwerveDrive {
    ...
    private static final double TRACKWIDTH_METERS = Units.inchesToMeters(24);
    private static final double WHEELBASE_METERS  = Units.inchesToMeters(24);
    ...
    // private static final double[] CANCODER_OFFSETS = { 74.2, 227, 273.9, 0 };
    private static final double[] CANCODER_OFFSETS = { 0, 0, 0, 0 };
    ...
    public void drive(double inputX, double inputY, double inputR) {
        ...
        for (int i = 0; i < cancoders.length; i++) {
            SmartDashboard.putNumber(
                    "Cancoder " + cancoders[i].getDeviceID() + " Position",
                    cancoders[i].getPosition());
        }
    }
    ...
    public double[] getCancoderPositions() {
        ...
        for (int i = 0; i < this.cancoders.length; i++) {
            positions[i] = this.cancoders[i].getPosition();
        }
        return positions;
    }
    ...
    public void setAngles(double degrees) {
        for (int i = 0; i < this.modules.length; i++) {
            this.modules[i].set(1.5, Units.degreesToRadians(degrees));
        }
    }
